...
class data_processing:
    ...
    def tracker_e2(moveID_df):
        ...
        for index, row in moveID_df.iterrows():
            ...
            goal_optimal = optimal_moves.select_optimal_moves(goal, currentConfig)
            goal_type = goal.split(" ")[0]

            print(f"for new ID = {anonID}, goal {goal_number}")
            for k in range(0, len(moveIDs)):
                ...
                id_df = pd.DataFrame({'anonID': [anonID]})
                id_df["importId"]= row["importId"]
                id_df["goal"] = goal
                id_df["goal_number"] = goal_number
                id_df["goal_type"] = goal_type
                id_df["optimal_moves"] = goal_optimal
                id_df["move_id"] = k
                id_df["move"] = str(m)
        ...
        individual_move_tracker.to_csv('../data/e2/final_tracker.csv', index=False)
    ...
    def first_architect_move(moveID_df):
        ...
        for index, row in moveID_df.iterrows():
            ...
            for m in u[1]:        
                ...
                move_id_df = pd.DataFrame({"ID": [anonID], "goal":[goal]})
                move_id_df["move"] = [m]
...
# individual move tracker code
moveID_df = pd.read_csv("../data/e2/cleaned_e2.csv", converters={"moveIDs": literal_eval, "config": literal_eval})
#data_processing.tracker_e2(moveID_df)
data_processing.first_architect_move(moveID_df)