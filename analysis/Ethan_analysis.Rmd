...
print(corrections_per_game)

# average corrections per game by goal_type
corrections_per_game %>%
  group_by(goal_type) %>%
  summarise(avg_corrections_per_game = mean(num_corrections), n_games = n()) %>%
  print()

# proportion of corrections by move type and goal type
proportion_corrections <- e1_move_list %>%
  group_by(goal_type, helper_move_utility = move_utility) %>%
  summarise(
    total_helper_moves = n(),
    corrections = sum(is_correction, na.rm = TRUE),
    prop_correction = corrections / total_helper_moves,
    .groups = "drop"
  )

print(proportion_corrections)
...
corrections %>%
  count(goal_type) %>%
  ggplot(aes(x = goal_type, y = n, fill = goal_type)) +
    geom_bar(stat = "identity") +
    labs(title = "Count of Corrections by Goal Type", x = "Goal Type", y = "Number of Corrections") +
    scale_fill_brewer(palette = "Set2") +
    theme_minimal()

# proportion of corrections by helper move utility and goal type
ggplot(proportion_corrections, aes(x = factor(helper_move_utility), y = prop_correction, fill = factor(helper_move_utility))) +
  geom_bar(stat = "identity", position = position_dodge(.8), width = .7) +
  facet_wrap(~ goal_type) +
  labs(title = "Proportion of Corrections by Move Utility and Goal Type", x = "Helper Move Utility", y = "Proportion Corrected") +
  scale_fill_brewer(palette = "Set2", name = "Move Utility") +
  theme_minimal()  
