...
corrections <- e1_move_list %>%
  ...
  ungroup() %>%
  filter(role == "helper", next_move_role == "architect", helper_move_to == arch_pickup) %>%
  select(ID, goal, goal_type, move_id, helper_move_to, arch_pickup, helper_move_utility = move_utility, arch_move_utility)
...
# utility of helper move preceding correction

corrections %>%
  count(goal_type, helper_move_utility) %>%
  ggplot(aes(x = as.factor(helper_move_utility), y = n, fill = as.factor(helper_move_utility))) +
  geom_bar(stat = "identity") +
  facet_wrap(~ goal_type) +
  labs(
    title = "Corrections by Helper Utility and Goal Type",
