...
e1_model_results <- read_csv("../data/e1/all_model_results.csv") %>%
  mutate(ID = as.character(ID), experiment = "E1")
e2_model_results <- read.csv("../data/e2/all_model_results.csv") %>%
  mutate(ID = as.character(ID), experiment = "E2")
...
# e1 comparison
e1_sum_NLL <- e1_model_results %>%
  group_by(goal_type, model) %>%
  summarise(total_NLL = sum(NLL), .groups = "drop")
print(e1_sum_NLL)

ggplot(e1_sum_NLL, aes(x = model, y = total_NLL, fill = model)) + geom_col() + 
  facet_wrap(~ goal_type) + 
  labs(title = "E1 Summed NLL per Model by Goal Type",
       x = "Model",
       y = "Total NLL") + 
  theme_minimal() +
...
# e2 comparison
e2_sum_NLL <- e2_model_results %>%
  group_by(goal_type, model) %>%
  summarise(total_NLL = sum(NLL), .groups = "drop")
print(e2_sum_NLL)

ggplot(e2_sum_NLL, aes(x = model, y = total_NLL, fill = model)) + geom_col() + 
  facet_wrap(~ goal_type) + 
  labs(title = "E2 Summed NLL per Model by Goal Type",
       x = "Model",
       y = "Total NLL") + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# e1 vs e2 comparison
combined <- bind_rows(e1_model_results, e2_model_results)

# compute mean NLL per ID, goal_type, model, and experiment
mean_NLL_per_id <- combined %>%
  group_by(experiment, goal_type, model, ID) %>%
  summarise(mean_NLL = mean(NLL), .groups = "drop")

# compute average across IDs
mean_NLL_summary <- mean_NLL_per_id %>%
  group_by(experiment, goal_type, model) %>%
  summarise(avg_NLL = mean(mean_NLL), .groups = "drop")

# plot
ggplot(mean_NLL_summary, aes(x = model, y = avg_NLL, fill = experiment)) +
  geom_col(position = position_dodge()) +
  facet_wrap(~ goal_type) +
  labs(
    title = "Average NLL per Model by Goal Type (E1 vs E2)",
    x = "Model",
    y = "Average NLL per ID"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
