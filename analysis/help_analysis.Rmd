...
e2_final_move_df = read_csv("../data/e2/cleaned_e2.csv") %>%
  rename(playerId = ID)

e3data = read_csv("../data/e2/e3a_data.csv") %>% 
  select(anonID, importId, goal, playerId,gameId) %>% distinct() %>%
  left_join(e2_final_move_df)

write.csv(e3data, file = "../data/e2/e3a_data_wide.csv", row.names = FALSE)
