...
class optimize:    
    ...
    def optimize_pragmatic_architect(moveID_df):
        ...
        return architect_optimized, (beta_opt[0][0], beta_opt[0][1])
    ...
    def optimize_literal_architect(moveID_df):
        ...
        final_beta_literal = None

        for ID in IDs:
            if(ID in opt_IDs):
                print(f"{ID} already optimized")
                beta_literal = architect_optimized[architect_optimized["ID"] == ID]["action_noise"].values[0]
                final_beta_literal = beta_literal
            else:
                print(f"optimizing architect for ID {ID}")
                ID_df = moveID_df.loc[moveID_df['ID'] == ID]
                goal_types = ID_df["goal_type"].unique()
                ...
                for goal_type in goal_types:
                    ...
                    beta_value = beta_opt[0][0]
                    beta_df = pd.DataFrame({'ID': [ID], 'goal_type': [goal_type], 'action_noise': beta_value})

                    architect_optimized = pd.concat([architect_optimized, beta_df], ignore_index=True)
                    final_beta_literal = beta_value

        # write to csv
        architect_optimized.to_csv('arch_lit_opt.csv', index=False)

        return architect_optimized, final_beta_literal
